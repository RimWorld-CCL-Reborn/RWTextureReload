...
{
    //  [StaticConstructorOnStartup] This class does not work yet...
    public static class TextureReloadV2
    {
        ...
        static TextureReloadV2()
        {
            ...
            typeof(Graphic).AllSubclassesNonAbstract().Do(t => 
                    harmony.Patch(AccessTools.Method(t, "DrawWorker"), null, new HarmonyMethod(typeof(TextureReloadV2), nameof(AddGraphic))));
        ...
        static void ReloadTextures(Dialog_DebugActionsMenu __instance) => 
            Traverse.Create(__instance as Dialog_DebugOptionLister).Method("DebugAction", "Reload Textures", new Action(() =>
            {
                ...
                graphicData.RemoveAll(wr => !wr.IsAlive);
                ...
                graphics.ForEach(wr =>
                {
                    ...
                    {
                        Log.Message("hey");
                        graphic.Init(new GraphicRequest(graphic.GetType(), graphic.data?.texPath ?? graphic.path, graphic.data != null ? ShaderDatabase.ShaderFromType(graphic.data.shaderType) : graphic.Shader,
                            graphic.data?.drawSize ?? graphic.drawSize, graphic.data?.color ?? graphic.color, graphic.data?.colorTwo ?? graphic.colorTwo, graphic.data, 0));
                    }
                });
                Log.Message(graphics.Count + " " + graphicData.Count);
            })).GetValue();
